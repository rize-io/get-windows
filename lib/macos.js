...
const checkForUrlInTitle = (result) => {
	...
	for (const subtitle of windowTitleAsArray) {
		if (regexURL.test(subtitle)) {
			result.url = subtitle;
			windowTitleUrlIndex = windowTitleAsArray.indexOf(subtitle);
		}
	}
...
module.exports.sync = (options) => {
	const stdout = childProcess.execFileSync(bin, getArguments(options), {
		encoding: "utf8",
	});
	return parseMac(stdout);
};

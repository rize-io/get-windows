...
// List of supported web browsers for active URL
...
try {
	let _path = path.join(__dirname, `../../../../.webpack/main/dlls/ActiveURL_${arch}.dll`);
	throw Error(__dirname);

	ActiveURL = new ffi.Library(_path, {
		GetChromeDetails: ['string', ['pointer']],
		GetFirefoxDetails: ['string', ['pointer']],
		GetEdgeDetails: ['string', ['pointer']],
		GetBraveDetails: ['string', ['pointer']],
		GetOperaIncognitoCheck: ['string', ['pointer']],
	});
} catch (error) {
	ActiveURL = null;
	console.error(error);
...
function windows() {
	...
	let windowTitle = wchar.toString(windowTextBufferClean);
	...
	try {
		...
		if (windowTitleUrlIndex > -1) {
			...
			windowTitle = [...arrayBeforeUrl, ...arrayAfterUrl].join(SEPARATOR);
			windowIncognito = false;
		} else if (ActiveURL && browserList.includes(processName)) {
			const browser = processName;

			if (browser === BRAVE) {
				browserData = ActiveURL.GetBraveDetails(activeWindowHandle);
			}

			if (browser === CHROME) {
				browserData = ActiveURL.GetChromeDetails(activeWindowHandle);
			}

			if (browser === FIREFOX) {
				browserData = ActiveURL.GetFirefoxDetails(activeWindowHandle);
			}

			if (browser === EDGE) {
				browserData = ActiveURL.GetEdgeDetails(activeWindowHandle);
			}

			if (browser === OPERA_1 || browser === OPERA_2) {
				browserData = ActiveURL.GetOperaIncognitoCheck(activeWindowHandle);
			}

			if (browserData) {
				const { url, incognito, error } = JSON.parse(browserData);
	...
	return {
		...
		url: windowURL,
		mode: windowIncognito ? 'incognito' : 'normal'
	};

...
const browserList = [CHROME, FIREFOX, EDGE];

let winurl;

try {
	let _path = path.join(__dirname, '../../../../dlls/BrowserURLs_' + arch + '.dll');

	if (!fs.existsSync(_path)) {
		_path = path.join(__dirname, '../../../dlls/BrowserURLs_' + arch + '.dll');
	}
	...
	winurl = null;
	console.error(error);
	throw new Error('Error parsing window URL');
}
...
function windows() {
	...
	try {
		if (winurl && browserList.includes(processName.toLowerCase())) {
			switch (processName.toLowerCase()) {
				case CHROME:
					windowURL = winurl.FetchChromeURL(activeWindowHandle);
					break;

				case FIREFOX:
					windowURL = winurl.FetchFirefoxURL(activeWindowHandle);
					break;

				case EDGE:
					windowURL = winurl.FetchEdgeURL(activeWindowHandle);
					break;
